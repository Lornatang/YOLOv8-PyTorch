...
class ThreadingLocked:
    ...
    Example:
        ```python
        from yolov8_pytorch.utils import ThreadingLocked
...
def get_user_config_dir(sub_dir="Ultralytics"):
    ...
    if not is_dir_writeable(path.parent):
        LOGGER.warning(f"WARNING ⚠️ user config directory '{path}' is not writeable, defaulting to '' or CWD."
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        from yolov8_pytorch.utils.torch_utils import torch_distributed_zero_first
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not (correct_keys and correct_types and correct_version):
                LOGGER.warning(
                    ...
                    "with your settings or a recent yolov8_pytorch package update. ")
                self.reset()
...
def deprecation_warn(arg, new_arg, version=None):
    ...
    LOGGER.warning(f"WARNING ⚠️ '{arg}' is deprecated and will be removed in 'yolov8_pytorch {version}' in the future. "
...
PREFIX = colorstr("YOLOv8-PyTorch: ")
SETTINGS = SettingsManager()  # initialize settings
DATASETS_DIR = Path(SETTINGS["datasets_dir"])  # global datasets directory
WEIGHTS_DIR = Path(SETTINGS["weights_dir"])  # global weights directory
RUNS_DIR = Path(SETTINGS["runs_dir"])  # global runs directory
ENVIRONMENT = platform.system()
TESTS_RUNNING = is_pytest_running()
set_sentry()
