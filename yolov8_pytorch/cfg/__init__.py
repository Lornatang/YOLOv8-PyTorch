...
from yolov8_pytorch.utils import (ASSETS, DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, RANK, RUNS_DIR,
...
def get_save_dir(args, name=None):
    ...
    if getattr(args, 'save_dir', None):
        ...
        project = args.project or ('tests/tmp/runs' if TESTS_RUNNING else RUNS_DIR) / args.task
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Example:
        ...
        ```
    """
    from yolov8_pytorch import hub
...
def entrypoint(debug=''):
    """
    This function is the yolov8_pytorch package entrypoint, it's responsible for parsing the command line arguments passed
    to the package.
    ...
    if 'rtdetr' in stem:  # guess architecture
        from yolov8_pytorch import RTDETR
        model = RTDETR(model)  # no task argument
    elif 'fastsam' in stem:
        from yolov8_pytorch import FastSAM
        model = FastSAM(model)
    elif 'sam' in stem:
        from yolov8_pytorch import SAM
        model = SAM(model)
    else:
